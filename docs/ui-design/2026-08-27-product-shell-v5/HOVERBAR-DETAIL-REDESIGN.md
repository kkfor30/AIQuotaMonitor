...
4. 每个账户组独立显示账户名、真实 `plan_level`、该套餐实际返回的窗口、重置时间和账户状态。Plus/Pro 默认 5 小时/7 天；Free 默认 30 天月窗口。
5. 默认只允许展示：`quota_window_5h`、`quota_window_7d`、`quota_window_30d`、`balance`、`plan_level`，以及平台/Source 状态。Free 缺少 5 小时/7 天窗口时不占行，只展示月窗口剩余。
